...
class MapBatchSelect(nn.Module):
    ...
    def forward_map_poses(self, map_coverages,plan_mask = None):
        if plan_mask is None:
            return map_coverages
        
        mask_t = torch.Tensor(plan_mask) == True
        mask_t = mask_t.cuda(0)
        covs_size = list(map_coverages.size())[1:]
        
        select_coverages = map_coverages[mask_t[:, np.newaxis, np.newaxis, np.newaxis].expand_as(map_coverages)].view([-1] + covs_size)
        
        return select_coverages

    def forward(self, instructions, images, map_poses, cam_poses, noisy_poses, start_poses, sent_embeds, plan_mask=None, show=""):
        if plan_mask is None:
            return  instructions, images, map_poses, cam_poses, noisy_poses, start_poses, sent_embeds, plan_mask

        mask_t = torch.Tensor(plan_mask) == True
        mask_t = mask_t.to(images.device)

        select_images, select_instructions, select_states = [], [], []
        # print(f"MASK_T: {mask_t}")
        for i in range(mask_t.size(0)):
            if mask_t[i] == True:
            #     select_images.append(images[i])
                select_instructions.append(instructions[i])
                select_images.append(images[i])
        # select_images = torch.stack(select_images)
        select_instructions = torch.stack(select_instructions)
        select_images = torch.stack(select_images)
        # maps_size = list(maps.size())[1:]
        # select_maps = maps[mask_t[:, np.newaxis, np.newaxis, np.newaxis].expand_as(maps)].view([-1] + maps_size)
        # covs_size = list(map_coverages.size())[1:]
        ...
        return select_instructions, select_images,select_poses, select_cam_poses, select_noisy_poses, select_start_poses, select_sent_embeds, mask_t