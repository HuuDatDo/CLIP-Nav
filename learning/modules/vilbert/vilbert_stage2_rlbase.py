...
###TODO(OK): Set up the dimension
class VilBert_Stage2_RLBase(nn.Module):
    def __init__(self, map_channels=1, map_struct_channels=1, map_size=32, crop_size=16, h1=8, structure_h1=8, h2=128, obs_dim=16, name="base"):
        super(VilBert_Stage2_RLBase, self).__init__()
        ...
        self.conv2 = nn.Conv2d(map_channels, h1, kernel_size=3, stride=2, padding=1, bias=True)

        self.structconv1 = nn.Conv2d(self.map_structure_channels, structure_h1, kernel_size=3, stride=2, padding=1, bias=True)
        self.structconv2 = nn.Conv2d(self.map_structure_channels, structure_h1, kernel_size=3, stride=2, padding=1, bias=True)
        ...
        # self.linear1z = nn.Linear(linear_in, h2)
        # self.linear2z = nn.Linear(h2 + linear_in, h2)
        ...
        self.norm2 = nn.InstanceNorm2d(h1)
        self.covnorm1 = nn.InstanceNorm2d(structure_h1)
        self.covnorm2 = nn.InstanceNorm2d(structure_h1)
    ...
    def forward(self, maps_r, maps_z, v_map_structure_r, z_map_structure_r):
        maps_r_cropped = maps_r.inner_distribution[:, :, self.crop_l:self.crop_r, self.crop_l:self.crop_r]
        maps_z_cropped = maps_z.inner_distribution[:, :, self.crop_l:self.crop_r, self.crop_l:self.crop_r]
        ...
        # 64x64 -> 16x16
        v_uncov_r_pooled = self.avgpool(v_map_structure_r)
        z_uncov_r_pooled = self.avgpool(z_map_structure_r)
        ...
        z= self.act(self.conv2(maps_z_cropped))
        z = self.norm2(z)
        
        x = torch.cat([x,z],dim=1)
        ###TODO(OK): Figure out the way to process the z_map to actions, concatenate or process it individually. How should we concat?
        # From 16x16 down to 8x8
        c = self.act(self.structconv1(v_uncov_r_pooled))
        c = self.covnorm1(c)
        d = self.act(self.structconv2(z_uncov_r_pooled))
        d = self.covnorm2(d)
