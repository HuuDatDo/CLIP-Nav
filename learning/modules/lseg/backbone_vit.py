...
def _make_vit_b_rn50_backbone(
    ...
    pretrained.model.patch_embed.backbone.stages[0].register_forward_hook(get_activation("resnet_1"))
    pretrained.model.patch_embed.backbone.stages[1].register_forward_hook(get_activation("resnet_2")) 
    ...
    #Split the ResNet for doing the mapping in between
    pretrained.model.first_patch_embed = nn.Sequential(*list(pretrained.model.patch_embed.backbone.stem),
                                      *list(pretrained.model.patch_embed.backbone.stages.children())[:1])
    pretrained.model.second_patch_embed = nn.Sequential(*list(pretrained.model.patch_embed.backbone.stages.children())[1:],
                                      pretrained.model.patch_embed.backbone.norm,
                                      pretrained.model.patch_embed.backbone.head)
    
    # print("FIRST PATCH EMBED:", pretrained.model.first_patch_embed)
    # print("SECOND_PATCH_EMBED:", pretrained.model.second_patch_embed)
    # print("ORI PATCH EMB:", pretrained.model.patch_embed)
    
    root_params = P.get_current_parameters()["ModelPVN"]
    params = root_params["Stage1"]
    
    pretrained.model.fpv_to_global_map = ClipFPVToGlobalMap(
            source_map_size=params["global_map_size"], world_size_px=params["world_size_px"], world_size_m=params["world_size_m"],
            res_channels=params["resnet_channels"], map_channels=params["feature_channels"],
            img_w=params["img_w"], img_h=params["img_h"], cam_h_fov=params["cam_h_fov"],
            domain="sim", img_dbg=False)
    
    pretrained.model.leaky_integrator_global_map = LeakyIntegratorGlobalMap(
            source_map_size=params["global_map_size"],
