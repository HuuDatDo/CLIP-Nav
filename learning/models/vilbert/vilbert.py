...
###TODO: EDIT THIS SO TO FIT THE RESNET OUTPUT
...
class BertPoseEmbeddings(nn.Module):
    """
    Construct the embeddings from  pose.
    """
    def __init__(self, config):
        super(BertPoseEmbeddings,self).__init__()
        self.pose_embeddings = nn.Linear(config.pose_feature, config.pose_hidden_size)
        self.LayerNorm = BertLayerNorm(config.pose_hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.pose_token_emb = nn.Parameter(torch.randn((1, config.pose_hidden_size), requires_grad=True))
        
    def forward(self, pose):
        embeddings = self.pose_embeddings(pose)
        embeddings = self.LayerNorm(embeddings)
        embeddings = self.dropout(embeddings)
        pos_token = self.pose_token_emb.expand(embeddings.shape[0],-1)
        # print(f"POS_TOKEN:{pos_token.size()}\nPOSE_EMBEDDINGS:{embeddings.size()}")
        embeddings = torch.stack([pos_token,embeddings])
        embeddings = embeddings.permute((1,0,2))
        #print(f"POSES_EMBEDDING_SIZE: {embeddings.size()}")
        return embeddings
...
class VILBertForSegmentation(nn.Module):
    def __init__(self, config, dim = 16, dropout_prob=0.1, default_gpu=True):
        super(VILBertForSegmentation, self).__init__()
        self.bert = BertModel(config).from_pretrained(
            pretrained_model_name_or_path='/home/harry/Desktop/Dat/drif-master/pytorch_model_9.bin',
            config = config,
    ...
    ###TODO: Change the shape of image from 12 to 14
    def forward(self,
                input_txt,
                input_imgs,
                pose,
        ...
        input_txt = input_txt.cuda(0)
        input_imgs = input_imgs.cuda(0)
        pose = pose.cuda(0)
        ...
        self.dropout.cuda(0)
        # self.mlp.cuda(0)
        # self.grounding_decoder.cuda(0)
        # self.vertical_decoder.cuda(0)
        ...
        return sequence_output_v, sequence_output_t