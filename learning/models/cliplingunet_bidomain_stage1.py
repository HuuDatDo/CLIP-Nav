...
class CLIPLingunetv2_Stage1_Bidomain(nn.Module):

    def __init__(self, run_name="", domain="sim"):

        super(CLIPLingunetv2_Stage1_Bidomain, self).__init__()
        ...
        self.cliplingunetv2 = CLIPLingUNetv2(head = nn.Sequential(
            Interpolate(scale_factor=1/2, mode="bilinear", align_corners=True),))
    ...
    def load_state_dict(self, state_dict, strict=True):
        super(CLIPLingunetv2_Stage1_Bidomain, self).load_state_dict(state_dict, strict)
    ...
    def forward(self, images, states, instructions, instr_lengths, save_img, count,
        ...
        self.cliplingunetv2 = self.cliplingunetv2.float().cuda(2)
        log_v_dist_p_select = self.cliplingunetv2(RS_P_select, textual_instructions, save_img, count, self.domain)
