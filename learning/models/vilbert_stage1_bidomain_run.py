...
from learning.modules.resnet.resnet_101 import ResNet101
from learning.modules.resnet.resnet_13_s import ResNet13S
# from learning.models.vilbert.map_embedding import MapsEmbedding
from learning.models.vilbert.vilbert import *
# from learning.models.vilbert.fast_rcnn import *
from learning.models.vilbert.mla_decoder import *
from learning.models.vilbert.pup_decoder import *
# from learning.models.vilbert.attn_decoder import *
...
class VilBert_Stage1_Bidomain(nn.Module):

    def __init__(self, run_name="", domain="sim"):

        super(VilBert_Stage1_Bidomain, self).__init__()
        ...
        #self.fast_rcnn = FeatureExtractor()
        # self.resnet13S = ResNet13S(channels =512)
        #self.resnet_101 = ResNet101(channels =512)
        self.map_embedding = MapsEmbedding(512)
        self.config  = BertConfig.from_json_file('/home/alladin/Dat/drif-master/bert_base_6layer_6conect.json')
        self.vilbert = VILBertForSegmentation(config= self.config)
        #self.decoder = Decoder(self.params["lingunet"])
        self.decoder = AttentionDecoder(self.params["lingunet"])
    ...
    def load_state_dict(self, state_dict, strict=True):
        super(VilBert_Stage1_Bidomain, self).load_state_dict(state_dict, strict)
    ...
    def forward(self, images, states, instructions, instr_lengths,
        ...
        select_instructions, select_states, S_W_select, SM_W_select, S_W_poses_select, cam_poses_select, noisy_start_poses_select, start_poses_select, sent_embeddings_select, mask_t = \
            self.batch_select(instructions, states, S_W, SM_W, S_W_poses, cam_poses, noisy_start_poses, start_poses, sent_embeddings, plan)
        ...
        #     RS_W_select = torch.cat([S_W_select, R_W_select], 1)
            #RS_W_select: torch.Size([3, 35, 32, 32])
        ...
        # ---------
        # features = self.resnet13S(select_images).reshape(batch_size,512, 20*28).permute(0,2,1)
        select_features, map16, map32 = self.map_embedding(S_W_select)
        batch_size = select_features.size(0)
        select_features = select_features.reshape(batch_size, 512, 8*8).permute(0,2,1)
        select_cam_poses = self.cam_poses_from_states(select_states)
        pose, orientation = select_cam_poses.get_tensor()
        poses = torch.cat([pose,orientation], dim=1).float() #size:[19,7]
        self.vilbert.cuda(0)
        sequence_output_v, sequence_output_t = self.vilbert(input_txt=select_instructions,
                                                            input_imgs = select_features,
                                                            pose= poses)
        
        self.decoder.cuda()
        inner_scores, outer_scores = self.decoder(sequence_output_v,sequence_output_t, map16, map32)
        # print(f"S_W_select:{S_W_select.size()}\n R_W_select:{R_W_select.size()}")

        inner_scores = inner_scores.cuda(0)
        outer_scores = outer_scores.cuda(0)

        log_v_dist_p_select = Partial2DDistribution(inner_scores, outer_scores)
        v_dist_p_poses_select = RS_P_poses_select
        # --------
        ...
        if self.get_iter() % 23 == 0 and not noshow:
            for i in range(S_W_select.shape[0]):
                Presenter().show_image(S_W_select.detach().cpu()[i,0:3], f"{self.domain}_s_w_select", scale=8, waitkey=1)
...
"""
WITHOUT SEMANTIC MAP AND PUP DECODE
Results: {'total_success': 48, 'total_fail': 122, 'total_segments': 170, '%success': 0.2823529411764706,
 'total_correct_landmarks': 0, 'visible_goal_segments': 120, 'invisible_goal_segments': 50, 'visible_goal_segments_correct': 43,
'invisible_goal_segments_correct': 5, 'emd': 72.58171992659364, 'visible_goal_emd': 41.70182921077958, 
'invisible_goal_emd': 30.87989071581406, 'total_dist': 137.25902320054416, 'last_dist': 0, 
'semd': 23.528764138097543, 'visible_goal_semd': 21.69413598940331, 'invisible_goal_semd': 1.834628148694231,
'nemd': 135.01940186111378, 'visible_goal_nemd': 82.24781106579417, 'invisible_goal_nemd': 52.77159079531973,
'avg_dist': 0.8074060188267304, '%correct_landmark': 0.0, 'visible_goal_success_rate': 0.35833333333333334,
'invisible_goal_success_rate': 0.1, 'visible_goal_rate': 0.7058823529411765, 'avg_emd': 0.4269512936858449,
'visible_goal_avg_emd': 0.34751524342316314, 'invisible_goal_avg_emd': 0.6175978143162812, 'avg_semd': 0.13840449492998555,
'visible_goal_avg_semd': 0.18078446657836092, 'invisible_goal_avg_semd': 0.036692562973884624, 'avg_nemd': 0.7942317756536105, 
'visible_goal_avg_nemd': 0.6853984255482847, 'invisible_goal_avg_nemd': 1.0554318159063947, 'median_dist': 0.7291760715046154}
"""