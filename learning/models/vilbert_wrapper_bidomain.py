...
# from learning.intrinsic_reward import zvel_reward
from learning.models.model_pvn_stage1_bidomain import PVN_Stage1_Bidomain
from learning.models.vilbert_stage1_bidomain import VilBert_Stage1_Bidomain
...
from learning.intrinsic_reward.height_reward import HeightReward
...
class VilBert_Wrapper_Bidomain(nn.Module):

    def __init__(self, run_name="", model_instance_name="only", oracle_stage1=False):
        super(VilBert_Wrapper_Bidomain, self).__init__()
        ...
        self.stage1_visitation_prediction = VilBert_Stage1_Bidomain(run_name, model_instance_name)
        ...
        if self.keyboard:
            ...
            self.stage2_action_generation = VilBert_Stage2_ActorCritic(run_name, model_instance_name)
        #else:
        ...
        self.height_reward = HeightReward(world_size_m = self.s1_params["world_size_px"],
                                          world_size_px = self.s1_params["world_size_m"], params = self.wrapper_params["wd_reward"])
    ...
    def load_models_from_file(self, stage1_file=None, stage2_file=None):
        ...
        if stage1_file:
            print("VilBert_Wrapper: Loading Stage 1")
            try:
        ...
        if stage2_file:
            print("VilBertWrapper: Loading Stage 2")
            try:
    ...
    def make_map_boundary(self):
        ...
        boundary[:,:,:,mapsize-1] = 1.0
        return boundary
###TODO: Change z to y axis LOL
###TODO: Consider the zvel reward since it's similar to the visitation reward and the stop reward is for both of it
    def calc_intrinsic_rewards(self, next_state, action, done, first):
        if self.v_dist_w is None or self.map_uncoverage_w is None:
            ...
            if self.s1_params.get("clip_observability") and self.wrapper_params.get("wasserstein_reward"):
                ...
                zvel_reward = self.zvel_reward(self.v_dist_z, cam_pos, action)
    ...
    #TODO: Edit the return for the stage 1
    ###TODO: Delete all the map that we don't need
    ...
    def get_action(self, state, instruction, sample=False, rl_rollout=False):
        ...
        if plan_now or True:
            ...
            self.log_v_dist_w, v_dist_w_poses, self.log_z_dist_w, z_dist_w_poses, rl_outputs = self.stage1_visitation_prediction(
            ...
            self.z_dist_w = self.log_z_dist_w.softmax()
            # TODO: Fix
        ...
        z = self.z_dist_w.inner_distribution
        ###TODO: Check if we need to set another layer for different parameters
        xr, r_poses = self.map_transformer_w_to_r(x, None, drn_poses)
        zr, r_poses = self.map_transformer_w_to_r(z, None, drn_poses)
        v_dist_r = Partial2DDistribution(xr, self.v_dist_w.outer_prob_mass)
        z_dist_r = Partial2DDistribution(zr, self.z_dist_w)
        
        structured_map_info_r, map_info_w = self.build_map_structured_input(self.map_uncoverage_w, drn_poses)

        ###TODO: Check the oracle stage
        if self.oracle_stage1:
            ...
            #z_dist_w_gt = aup.resolve_and_get_ground_truth_static_global(env_id, set_idx, seg_idx, self.s1_params["global_map_size"], self.s1_params["world_size_px"]).to(images_fpv.device)
        ...
        if self.rl:
            ...
            xvel_dist, yawrate_dist, zvel_dist, stop_dist, value = self.stage2_action_generation(v_dist_r, structured_map_info_r, eval=True)
            #TODO: Add the zvel to the action
            self.actprof.tick("stage2")
            if sample:
                xvel, yawrate, zvel, stop = self.stage2_action_generation.sample_action(xvel_dist, yawrate_dist, zvel_dist, stop_dist)
            else:
                xvel, yawrate, zvel, stop = self.stage2_action_generation.mode_action(xvel_dist, yawrate_dist, zvel_dist, stop_dist)
            ...
            act = np.concatenate([xvel, np.zeros(xvel.shape), yawrate, zvel, stop])
            ...
            if rl_rollout:
                ...
                ###TODO(OK): Add the z_data to the viz_data
                ...
                viz_data["z_dist_r_inner"] = z_dist_r.inner_distribution[0].detach().cpu().numpy()
                viz_data["z_dist_r_outer"] = z_dist_r.outer_prob_mass[0].detach().cpu().numpy()
                viz_data["z_dist_w_inner"] = self.z_dist_w.inner_distribution[0].detach().cpu().numpy()
                viz_data["z_dist_w_outer"] = self.z_dist_w.outer_prob_mass[0].detach().cpu().numpy()
    ...
    ###TODO: Also edit the unbatch function
    ...
    #TODO: Check the training variables
    def sup_loss_on_batch(self, batch, eval, halfway=False):
        ...
        ###TODO(OK): Add the zvel, check the action_generation and the index
        xvel_dist, yawrate_dist, zvel_dist, stop_dist, value_pred = self.stage2_action_generation(v_dist_r_gt_masked, structured_map_info_r, eval=False)
        xvel_logprob = xvel_dist.log_probs(actions_gt_select[:,0])
        yawrate_logprob = yawrate_dist.log_probs(actions_gt_select[:,2])
        zvel_logprob = zvel_dist.log_probs(actions_gt_select[:,3])
        # TODO: Figure out why this doesn't already sum
        stop_logprob = stop_dist.log_probs(actions_gt_select[:,4]).sum()
        ...
        avg_zvel_logprob = zvel_logprob/ batch_size
        ...
        squared_zvel_dst = ((zvel_dist.mean - actions_gt_select[:,3]) ** 2).mean()
        
        #action_loss = -avg_stop_logprob + squared_xvel_dst + squared_yawrate_dst
        action_loss = -avg_stop_logprob + squared_xvel_dst + squared_yawrate_dst + squared_zvel_dst
        ...
        self.stage2_action_generation.writer.add_scalar(prefix + "/avg_zvel_logprob", avg_zvel_logprob.data.cpu().item(), self.stage2_action_generation.get_iter())
        ...
        self.stage2_action_generation.writer.add_scalar(prefix + "/squared_zvel_dst", squared_zvel_dst.data.cpu().item(), self.stage2_action_generation.get_iter())
