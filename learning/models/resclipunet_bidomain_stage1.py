...
from learning.modules.clipunet.clip_lingunetv2 import CLIPLingUNetv2
from learning.modules.clipunet.resclipunet import CLIPUnet, CLIPUnetv2
...
class CLIPUnet_Stage1_Bidomain(nn.Module):

    def __init__(self, run_name="", domain="sim"):

        super(CLIPUnet_Stage1_Bidomain, self).__init__()
        ...
        self.clipunet = CLIPUnet(self.params["lingunet"])
        # self.clipunet = CLIPUnetv2(self.params["lingunet"])
    ...
    def load_state_dict(self, state_dict, strict=True):
        super(CLIPUnet_Stage1_Bidomain, self).load_state_dict(state_dict, strict)
    ...
    def forward(self, images, states, instructions, instr_lengths, save_img, count,
        ...
        for i in range(min(len(instructions[0])-1,69)):
            textual_instructions += self.token2word[int(instructions[0][i])]
            textual_instructions += " "
        if len(textual_instructions.split(" ")) > 65:
            print(textual_instructions)
        self.clipunet = self.clipunet.float()
        
        log_v_dist_p_select = self.clipunet(RS_P_select, textual_instructions, save_img, count, self.domain)
        # ---------
        ...
        if save_img:
            for i in range(S_W_select.shape[0]):
                Presenter().save_image(S_W_select.detach().cpu()[i,0:3], count, f"{self.domain}_s_w_select", scale=4)
                Presenter().save_image(lsfm(log_v_dist_s_select.inner_distribution).detach().cpu()[i], count, f"{self.domain}_v_dist_s_select", scale=4)
                Presenter().save_image(lsfm(log_v_dist_p_select.inner_distribution).detach().cpu()[i], count, f"{self.domain}_v_dist_p_select", scale=4)
                Presenter().save_image(RS_P_select.detach().cpu()[i,0:3], count, f"{self.domain}_rs_p_select", scale=4)
                break
    ...
    def unbatch(self, batch, halfway=False):
        ...
        if not halfway:
            ...
            resnet_factor = 4 #self.img_to_features_w.img_to_features.get_downscale_factor()
