...
from learning.models.vilbert.attention_decoder import *
from learning.models.clip.model import *
from learning.models.clip.decoder import *
...
class VilBert_Stage1_Bidomain(nn.Module):
    ...
    def __init__(self, run_name="", domain="sim"):
        ...
        self.config  = BertConfig.from_json_file('/home/harry/Desktop/Dat/demo/vilbert-multi-task/config/bert_base_2layer_2conect.json')
        self.vilbert = VILBertForSegmentation(config= self.config)
        # self.decoder = Decoder(self.params["lingunet"])
        # self.decoder = PUPDecoder(self.params["lingunet"])
    ...
    def forward(self, images, states, instructions, instr_lengths,
        ...
        # img_grid = img_grid.permute(1,2,0).cpu()
        # print(f"IMG_GRID :{img_grid.size()}")
        # plt.imshow(img_grid)
        # self.writer.add_image("Input Image", img_grid, 0)
        ...
        F_W, M_W, features = self.img_to_features_w(images, cam_poses, sent_embeddings, self.tensor_store, show="", halfway=halfway)
        ...
        select_features, select_instructions, select_states, S_W_select, SM_W_select, S_W_poses_select, cam_poses_select, noisy_start_poses_select, start_poses_select, sent_embeddings_select, mask_t = \
            self.batch_select(features, instructions, states, S_W, SM_W, S_W_poses, cam_poses, noisy_start_poses, start_poses, sent_embeddings, plan)
...
# S_W_select: torch.Size([3, 32, 32, 32])
# R_W_select: torch.Size([3, 3, 32, 32])
# RS_W_select: torch.Size([3, 35, 32, 32])
# RS_S_select: torch.Size([3, 35, 64, 64])
# RS_P_SELECT: torch.Size([3, 35, 64, 64])
# S_W_select: torch.Size([6, 32, 32, 32])
# R_W_select: torch.Size([6, 3, 32, 32])
# RS_W_select: torch.Size([6, 35, 32, 32])
# RS_S_select: torch.Size([6, 35, 64, 64])
# RS_P_SELECT: torch.Size([6, 35, 64, 64])
        ...
        select_features = select_features.reshape(batch_size, 512, 12*16).permute(0,2,1)
        ...
        self.decoder.cuda(0)
        inner_scores, outer_scores = self.decoder(sequence_output_v,sequence_output_t)
        S_W_select = S_W_select.cuda(0)
        R_W_select = R_W_select.cuda(0)
        sent_embeddings_pp = sent_embeddings_pp.cuda(0)
