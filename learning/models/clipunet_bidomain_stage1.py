...
from learning.modules.lseg.backbone_vit import _make_pretrained_clip_vitl16_384
from learning.modules.lseg.lseg_net import LSeg
from learning.models.clip.decoder import *
from learning.modules.clipunet.clip_lingunet import CLIPLingUNet
...
class CLIPLingUNet_Stage1_Bidomain(nn.Module):

    def __init__(self, run_name="", domain="sim"):

        super(CLIPLingUNet_Stage1_Bidomain, self).__init__()
        ...
        self.clip_lingunet = CLIPLingUNet(head = nn.Sequential(
            Interpolate(scale_factor=1, mode="bilinear", align_corners=True),))
    ...
    def load_state_dict(self, state_dict, strict=True):
        super(CLIPLingUNet_Stage1_Bidomain, self).load_state_dict(state_dict, strict)
    ...
    def reset(self):
        ...
        self.clip_lingunet.pretrained.model.leaky_integrator_global_map.reset()
        self.prev_instruction = None
    ...
    def forward(self, images, states, instructions, instr_lengths,
        ...
        select_instructions, select_images ,S_W_poses_select, cam_poses_select, noisy_start_poses_select, start_poses_select,sent_embeddings_select, mask_t = \
            self.batch_select(instructions, images, S_W_poses, cam_poses, noisy_start_poses, start_poses,sent_embeddings, plan)
        
        textual_instructions = ""
        for i in range(min(len(select_instructions[0])-1,69)):
            textual_instructions += self.token2word[int(select_instructions[0][i])]
            textual_instructions += " "
        ...
        if halfway == "observability":
            map_uncoverage_w = 1 - SM_W_select
            return map_uncoverage_w

        # Throw away those timesteps that don't correspond to planning timesteps
        # select_instructions, select_images , cam_poses_select, noisy_start_poses_select, start_poses_select, mask_t = \
        #     self.batch_select(instructions, images, cam_poses, noisy_start_poses, start_poses, plan)
        ...
        #Convert back to a text to use clip tokenize
        # print("select_instructions: ",select_instructions)
        # textual_instructions = ""
        # for i in range(min(len(select_instructions[0])-1,76)):
        #     textual_instructions += self.token2word[int(select_instructions[0][i])]
        #     textual_instructions += " "
        # print("textual_instructions: ",textual_instructions)

        # chunk = self.extractor(textual_instructions)
        
        # self.lseg = self.lseg.float().cuda(1)
        # logit_seg_masks = self.lseg(select_images, textual_instructions, cam_poses_select)
        
        # logit_seg_masks = logit_seg_masks.cuda(0)
        # log_v_dist_p_select, v_dist_p_poses_select = self.path_predictor_lingunet(S_W_select, sent_embeddings_pp, RS_P_poses_select, tensor_store=self.tensor_store)
        # --------
