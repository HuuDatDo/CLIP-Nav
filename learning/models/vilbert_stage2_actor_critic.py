...
#TODO: Add the zvel action based on the distribution
class VilBert_Stage2_ActorCritic(nn.Module):

    def __init__(self, run_name="", model_instance_name="only"):

        super(VilBert_Stage2_ActorCritic, self).__init__()
        self.model_name = "vilbert_stage2_ac"
        ...
        ###TODO: Check if we need parameters
    ...
    def forward(self, v_dist_r, z_dist, map_structure_r, eval=False, summarize=False):
        ACPROF = False
        ...
        if isinstance(v_dist_r, list):
            ...
            v_map_structure_r = torch.stack(map_structure_r, dim=0)

        ###TODO: check the map_structure for
        if isinstance(z_dist, list):
            inners = torch.cat([m.inner_distribution for m in z_dist], dim=0)
            outers = torch.cat([m.outer_prob_mass for m in z_dist], dim=0)
            z_dist = Partial2DDistribution(inners, outers)
            z_map_structure_r = torch.stack(map_structure_r, dim=0)
        
        if self.ignore_struct:
            v_map_structure_r = torch.zeros_like(v_map_structure_r)
            z_map_structure_r = torch.zeros_like(z_map_structure_r)
        ###TODO: generate action with zvel
        #We might process the z and v individually
        prof.tick("ac:inputs")
        avec = self.action_base(v_dist_r, z_dist, v_map_structure_r, z_map_structure_r)
        ...
        zvel_mean = action_scores[:,6]
        zvel_std = F.softplus(action_scores[:,8])
        zvel_dist = FixedNormal(zvel_mean, zvel_std)

        # Skew it towards not stopping in the beginning
        stop_logits = action_scores[:, 9]
        ...
        return xvel_dist, yawrate_dist, zvel_dist, stop_dist, value_pred

    def sample_action(self, xvel_dist, yawrate_dist, zvel_dist, stop_dist):
        # Sample action from the predicted distributions
        xvel_sample = xvel_dist.sample()
        yawrate_sample = yawrate_dist.sample()
        zvel_sample = zvel_dist.sample()
        stop = stop_dist.sample()
        return xvel_sample, yawrate_sample, zvel_sample, stop

    def mode_action(self, xvel_dist, yawrate_dist, zvel_dist, stop_dist):
        xvel_sample = xvel_dist.mode()
        yawrate_sample = yawrate_dist.mode()
        zvel_sample = zvel_dist.mode()
        stop = stop_dist.mean
        return xvel_sample, yawrate_sample, zvel_sample, stop

    def action_logprob(self, xvel_dist, yawrate_dist, zvel_dist, stop_dist, xvel, yawrate, zvel, stop):
        xvel_logprob = xvel_dist.log_prob(xvel)
        yawrate_logprob = yawrate_dist.log_prob(yawrate)
        zvel_logprob = zvel_dist.log_prob(zvel)
    ...
    def evaluate_actions(self, maps_r_batch, map_cov_r_batch, hidden_states_batch, masks_batch, actions_batch, global_step):
        xvel_dist, yawrate_dist, zvel_dist, stop_dist, value_pred = self(maps_r_batch, map_cov_r_batch, summarize=True)

        # X-vel, yaw rate, and zvel. Drop sideways velocity
        x_vel = actions_batch[:, 0]
        yawrate = actions_batch[:, 2]
        zvel = actions_batch[:, 3]
        stops = actions_batch[:, 4]

        # Get action log-probabilities
        x_vel_log_probs, yawrate_log_probs, zvel_log_probs, stop_log_probs = \
            self.action_logprob(xvel_dist, yawrate_dist, zvel_dist, stop_dist, x_vel, yawrate, zvel, stops)

        x_vel_entropy = xvel_dist.entropy().mean()
        yawrate_entropy = yawrate_dist.entropy().mean()
        zvel_entropy = zvel_dist.entropy().mean()
        stop_entropy = stop_dist.entropy().mean()

        entropy = x_vel_entropy + yawrate_entropy + zvel_entropy + stop_entropy
        log_probs = x_vel_log_probs + yawrate_log_probs + zvel_log_probs + stop_log_probs
        ...
        if i % 17 == 0:
            ...
            if ACT_DIST == "normal":
                ...
                zvel_mean = zvel_dist.mean.detach().cpu().mean(dim=0)
                zvel_std = zvel_dist.stddev.detach().cpu().mean(dim=0)
                ...
                self.writer.add_scalar(f"{self.model_name}/zvel_mean", zvel_mean.item(), i)
                self.writer.add_scalar(f"{self.model_name}/zvel_std", zvel_std.item(), i)
                ...
                zvel_alpha = zvel_dist.concentration0.detach().cpu().mean(dim=0)
                zvel_beta = zvel_dist.concentration1.detach().cpu().mean(dim=0)
                ...
                self.writer.add_scalar(f"{self.model_name}/zvel_alpha",zvel_alpha.item(),i)
                self.writer.add_scalar(f"{self.model_name}/zvel_beta", zvel_beta.item(),i)
        self.inc_iter()
